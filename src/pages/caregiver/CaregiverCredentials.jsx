...
const CaregiverCredentials = () => {
  ...
  const displayedFiles = (kind, existing) =>
    pendingFiles[kind] ? [pendingFiles[kind]] : existing ? [existing] : [];
  ...
  const uploadPendingFiles = async () => {
    ...
    try {
      ...
      setNotice(
        nextRecord.credentialsCompleted
          ? "Required credentials uploaded."
          : "Draft saved. Resume and both NID sides are required to continue.",
      );
      return nextRecord;
    } catch (uploadError) {
  ...
  const continueToAssessment = async () => {
    ...
    if (nextRecord?.credentialsCompleted) {
      ...
      setError(
        "Upload your resume and both sides of your NID card before continuing.",
      );
    }
  };
  ...
  return (
    <OnboardingLayout activeStep="credentials">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <FileUpload
          ...
          files={displayedFiles(
            "referenceLetter",
            record.credentials.referenceLetter,
          )}
          onChange={(event) =>
            chooseFile("referenceLetter", event.target.files[0])
          }
          onClear={() =>
            clearFile("referenceLetter", record.credentials.referenceLetter)
          }
        />
      </div>
      ...
      <section className="mt-8">
        ...
        <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
          {licenses.map(([name, detail, color]) => {
            ...
            const displayed =
              pendingFiles[kind] || record.credentials.licenses[name];
            return (
              <article
                className="overflow-hidden rounded-lg border border-[#c3c6d6] bg-white"
                key={name}
              >
                <div className="h-1.5" style={{ backgroundColor: color }} />
                <div className="p-5">
                  <h3 className="font-semibold">{name}</h3>
                  <p className="mt-2 min-h-10 text-xs text-[#434654]">
                    {detail}
                  </p>
                  <label
                    className="mt-4 block cursor-pointer rounded border border-[#737685] px-4 py-3 text-center text-sm font-semibold"
                    style={{ color }}
                  >
                    {displayed ? (
                    ...
                    <input
                      ...
                      accept=".pdf,image/*"
                      onChange={(event) =>
                        chooseFile(kind, event.target.files[0])
                      }
                    />
                  </label>
                  {displayed && (
                    <button
                      className="mt-3 w-full text-xs font-semibold text-red-700"
                      type="button"
                      onClick={() =>
                        clearFile(kind, record.credentials.licenses[name])
                      }
                    >
                      Remove
                    </button>
      ...
      <div className="mt-9 flex flex-col-reverse justify-between gap-3 border-t border-[#c3c6d6] pt-7 sm:flex-row">
        ...
        >
          {saving ? "Uploading…" : "Save Progress"}
        </button>
        <button
